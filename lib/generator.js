...
function ruleNameFromUrl(url) {
  try {
    ...
    const file  = parts[parts.length - 1] || parts[parts.length - 2] || 'rule'
    // Strip extension, sanitise to [a-zA-Z0-9_-]
    return file.replace(/\.[^.]+$/, '').replace(/[^a-zA-Z0-9_-]/g, '_') || 'rule'
  } catch {
    return 'rule'
  }
}
...
function buildProxyGroupsFromIni(iniGroups, proxyNames) {
  ...
  return iniGroups.map(g => {
    if (g.type === 'select') {
      return { name: g.name, type: 'select', proxies: g.proxies }
    }
...
function buildRulesFromIni(rulesets, customRules) {
  const ruleProviders = {}
  const rules         = []
 ...
 */
export function generateClashConfigFromIni(proxies, parsedIni, customRules = [], templateUrl = '') {
  const proxyNames     = proxies.map(p => p.name)
  const builtGroups    = buildProxyGroupsFromIni(parsedIni.proxyGroups, proxyNames)
  const { ruleProviders, rules } = buildRulesFromIni(parsedIni.rulesets, customRules)
