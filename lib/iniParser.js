...
function _parseProxyGroup(val, out) {
  ...
  if (type === 'select') {
    // parts[2..] are either:
    //   []GroupName  → reference to another proxy group / policy (DIRECT, REJECT, etc.)
    //   regex        → filter pattern applied against actual proxy node names (e.g. .*)
    // We keep the distinction by preserving the [] prefix on group refs.
    const proxies = parts
      .slice(2)
      .filter(Boolean)
      .map(p => (p.startsWith('[]') ? p.slice(2) : { filter: p }))
